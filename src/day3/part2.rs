        ...
        for col_idx in 0..col_len {
            if grid[row_idx][col_idx].value == '*' {
                ...
                if is_unvisited_digit(&grid, row_idx as i16 - 1, col_idx as i16) {
                    if let Some(neighboring_number) = try_visiting_number(&mut grid[row_idx.saturating_sub(1)], col_idx) {
                        neighbors.push(neighboring_number);
                    }
                }

                // Northeast
                if is_unvisited_digit(&grid, row_idx as i16 - 1, col_idx as i16 + 1) {
                    if let Some(neighboring_number) = try_visiting_number(&mut grid[row_idx.saturating_sub(1)], col_idx.saturating_add(1)) {
                        neighbors.push(neighboring_number);
                    }
                }
                ...
                if is_unvisited_digit(&grid, row_idx as i16, col_idx as i16 + 1) {
                    if let Some(neighboring_number) = try_visiting_number(&mut grid[row_idx], col_idx.saturating_add(1)) {
                        neighbors.push(neighboring_number);
                    }
                }
                ...
                // Southeast
                if is_unvisited_digit(&grid, row_idx as i16 + 1, col_idx as i16 + 1) {
                    if let Some(neighboring_number) = try_visiting_number(&mut grid[row_idx.saturating_add(1)], col_idx.saturating_add(1)) {
                        neighbors.push(neighboring_number);
                    }
                }
                ...
                // South
                if is_unvisited_digit(&grid, row_idx as i16 + 1, col_idx as i16) {
                    if let Some(neighboring_number) = try_visiting_number(&mut grid[row_idx.saturating_add(1)], col_idx) {
                        neighbors.push(neighboring_number);
                    }
                }
                ...
                // Southwest
                if is_unvisited_digit(&grid, row_idx as i16 + 1, col_idx as i16 - 1) {
                    if let Some(neighboring_number) = try_visiting_number(&mut grid[row_idx.saturating_add(1)], col_idx.saturating_sub(1)) {
                        neighbors.push(neighboring_number);
                    }
                }
                ...
                // West
                if is_unvisited_digit(&grid, row_idx as i16, col_idx as i16 - 1) {
                    if let Some(neighboring_number) = try_visiting_number(&mut grid[row_idx], col_idx.saturating_sub(1)) {
                        neighbors.push(neighboring_number);
                    }
                }
                ...
                // Northwest
                if is_unvisited_digit(&grid, row_idx as i16 - 1, col_idx as i16 - 1) {
                    if let Some(neighboring_number) = try_visiting_number(&mut grid[row_idx.saturating_sub(1)], col_idx.saturating_sub(1)) {
                        neighbors.push(neighboring_number);
                    }
                }

                if neighbors.len() == 2 {
                    sum += neighbors.into_iter().reduce(|a, b| a * b).unwrap();
                }
            }
        }
    }
...
fn try_visiting_number(row: &mut Vec<Cell>, col_start_idx: usize) -> Option<i32> {
    ...
    Some(String::from_iter(num_vec).parse::<i32>().unwrap())
}
...
mod tests {
    ...
    fn test_sum() {
        ...
        assert_eq!(467835, calculate_sum(&mut grid));
    }
